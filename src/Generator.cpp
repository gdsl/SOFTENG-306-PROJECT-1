...
void Generator::loadOrchard()
{
	...
	int rowCount, rowLength, rowWidth, trunkPoleSpacing;
...
void Generator::loadRobots()
{
	...
	for (int i = 0; i < pickerNumber; i++) {
		...
		outfile << "PickerRobot( pose [ " << x << " " << y << " 0 0 ] name \"Picker" << i+1 << "\" color \"random\")" << endl;
	}
	...
	for (int i = 0; i < carrierNumber; i++) {
		...
		outfile << "CarrierRobot( pose [ " << x << " " << y << " 0 0 ] name \"Carrier" << i+1 << "\" color \"random\")" << endl;
	}
