    ...
    for (int i = 0; i < columnCount; i++) {
        ...
        for (int j = 0; j < model.rowCount+1; j++) {
		    ...
		    if (i == 0 && j < model.rowCount) {
		        ...
		        beaconPositions.pushback(y-model.rowWidth/2.0);
...
void Generator::writeLaunchFile(){
    ...
    for (int i = 0; i < totalObjects; i++) {
        ...
        if (i < model.weed) { //weeds
            ...
            pickerRobotsPositions.pop();
            int xPathPos = pickerPathPositions.front();
            pickerPathPositions.pop();
            int yPathPos = pickerPathPositions.front();
            oss << xPos << " " << yPos << " " << xPathPos << " " << yPathPos << " " << model.rowWidth;
            ...
            int firstPicker = model.weed + model.beacons;
            int lastCarrier = model.weed + model.beacons + model.pickerRobots + model.carrierRobots - 1;
